...
function wirePanels() {
  ...
  // ❌ ลบส่วนนี้ (ปุ่ม menu ถูกใช้โดย sidebar แล้ว)
...
function createNewCanvas(w, h) {
  // ✅ ซ่อนหน้า Home และ New Canvas Page (ถ้ามี)
...
function initHome() {
  ...
  // ─── ปุ่มสร้างโปรเจกต์ใหม่ → เปิดหน้า New Canvas (เต็มหน้า) ───
  const newBtn = document.getElementById('home-btn-new');
  if (newBtn) {
    newBtn.addEventListener('click', () => {
      // ❌ ลบ openDialog(els.dialogNew); ออก
      // ✅ ใช้ openNewCanvasPage() แทน
      openNewCanvasPage();
    });
  }
...
function initNewCanvasPage() {
  ...
  sizeBtns.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      newCanvasSelectedSize = { w: size, h: size };
      newCanvasIsCustom = false;
      hideNewCanvasWarning();
      
      // ✅ สร้างเลย (ไม่ต้องกดปุ่มยืนยัน)
      createFromNewCanvas(size, size);
    });
  });
  ...
  if (customBtn) {
    customBtn.addEventListener('click', () => {
      ...
      customBtn.classList.add('selected');
      customRow.hidden = false;
      newCanvasIsCustom = true;
      newCanvasSelectedSize = null;
      setTimeout(() => document.getElementById('new-canvas-w')?.focus(), 100);
      checkNewCanvasWarning();
    });
  }
  ...
  if (createBtn) {
    createBtn.addEventListener('click', () => {
      const w = clamp(parseInt(document.getElementById('new-canvas-w')?.value, 10) || 32, 1, 2048);
      const h = clamp(parseInt(document.getElementById('new-canvas-h')?.value, 10) || 32, 1, 2048);
      createFromNewCanvas(w, h);
    });
  }

  // ─── ตรวจสอบ warning ───
  const wInput = document.getElementById('new-canvas-w');
  const hInput = document.getElementById('new-canvas-h');
  if (wInput) wInput.addEventListener('input', checkNewCanvasWarning);
  if (hInput) hInput.addEventListener('input', checkNewCanvasWarning);

  // ─── Enter key ───
  if (wInput) wInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') createBtn?.click(); });
  if (hInput) hInput.addEventListener('keydown', (e) => { if (e.key === 'Enter') createBtn?.click(); });
}
...
function openNewCanvasPage() {
  ...
  const page = document.getElementById('new-canvas-page');
  if (page) {
    // ✅ ลบ hidden attribute และ class hidden
    ...
    console.warn('new-canvas-page not found');
  }
}