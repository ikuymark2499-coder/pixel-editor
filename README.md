...
- "Open งานเดิม" (open existing work) is implemented as a local project
  ...
  equivalent.
- Canvas sizes are capped at 256×256 per the brief; the pixel-buffer and
  coordinate-mapping approach scales to larger sizes without a rewrite if
  that cap is lifted later.
